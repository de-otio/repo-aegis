...
const KNOWN_WORKFLOW_HASHES: readonly string[] = [
  // Pre-H2: no --no-hooks-check flag on the audit line (the hooks
  // liveness check didn't exist yet). Kept so a workflow installed by
  // an older repo-aegis is still recognised as "ours" (not
  // user-modified) after this upgrade — `install ci` was never run
  // again just because the CLI got smarter about hooks, so old
  // installs must remain uninstall-eligible.
